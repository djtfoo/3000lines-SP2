...
void Mtx44::SetToReflection(float axisX, float axisY)
{
    if (axisX == 1) {   //reflect about x-axis
        a[5] = -1;
    }
    if (axisY == 1) {   //reflect about y-axis
        a[0] = -1;
    }
}

void Mtx44::SetToShear(float shearAngle, float X, float Y, float Z)
{
    if (X == 1) {
        a[4] = tan(Math::PI / 180 * shearAngle);
        a[6] = 0;
    }
    else if (Y == 1) {
        a[1] = tan(Math::PI / 180 * shearAngle);
        a[2] = 0;
    }
    else if (Z == 1) {
        a[8] = tan(Math::PI / 180 * shearAngle);
        a[9] = 0;
    }
    
}

void Mtx44::setScale(Mtx44& model, Mtx44& scale, float scaleX, float scaleY, float scaleZ)
{
    scale.SetToIdentity();
...
void Mtx44::setRotation(Mtx44& model, Mtx44& rotate, float rotateAngle)
{
...
void Mtx44::setTranslation(Mtx44& model, Mtx44& translate, float translateX, float translateY, float translateZ)
{
    translate.SetToIdentity();
    translate.SetToTranslation(translateX, translateY, translateZ);
    model = translate * model;
}

void Mtx44::setReflection(Mtx44& model, Mtx44& reflect, float reflectX, float reflectY)
{
    reflect.SetToIdentity();
    reflect.SetToReflection(reflectX, reflectY);
    model = reflect * model;
}

void Mtx44::setShear(Mtx44& model, Mtx44& shear, float shearAngle, float X, float Y, float Z)
{
    shear.SetToIdentity();
    shear.SetToShear(shearAngle, X, Y, Z);
    model = shear * model;
}

void Mtx44::SetTo3DReflection(float axisX, float axisY, float axisZ)
{
    if (axisX == 1) {   //reflect about yz-plane
        a[0] = -1;
    }
    if (axisY == 1) {   //reflect about xz-plane
        a[5] = -1;
    }
    if (axisZ == 1) {   //reflect about xy-plane
        a[10] = -1;
    }
}